...
extension KeyCommand.Key {
  ...
  /// A string representation of the key, intended for display purposes.
  ...
  /// A string representation of the key that can be used as a key equivalent in a menu item.
  ...
  /// Runs a check against all the possible values of `Key`, and if a case
  /// is found for which `predicate` evaluates to `true`, creates a value
  /// initialized to that case.
  init?(matching predicate: (Self) throws -> Bool) rethrows {
    guard let key = try Self.allCases.first(where: predicate) else {
      return nil
    }
    self = key
  }
  ...
  /// Runs a check against all the possible values of `Key`, and if a case
  /// is found whose raw value matches the given integer, creates a value
  /// initialized to that case.
  init?(_ rawValue: Int) {
    self.init { $0.rawValue == rawValue }
  }

  /// Runs a check against all the possible values of `Key`, and if a case
  /// is found whose key equivalent matches the given string, creates a value
  /// initialized to that case.
  init?(keyEquivalent: String) {
    self.init { $0.keyEquivalent == keyEquivalent }
  }
