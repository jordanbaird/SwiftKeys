...
extension KeyCommand.Key {
    ...
    public var stringValue: String {
        customStringValue ?? keyEquivalent
    }
}

// MARK: Key Initializers
extension KeyCommand.Key {
    /// Runs a check against all possible cases, and if a case is
    /// found for which `predicate` evaluates to `true`, creates
    /// a value initialized to that case.
    ...
    /// Runs a check against all possible cases, and if a case is
    /// found whose key equivalent matches the given string, creates
    /// a value initialized to that case.
    init?(keyEquivalent: String) {
        self.init { key in
            key.keyEquivalent == keyEquivalent
        }
    }

    /// Creates a key that matches the specified raw value.
